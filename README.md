...
El objetivo es dEl sistema analizado corresponde a un proceso de liberación controlada de fármacos desde hidrogeles poliméricos inteligentes, materiales capaces de absorber agua y liberar un compuesto terapéutico de manera gradual.

En esta práctica se utilizaron datos experimentales de cuatro formulaciones de hidrogeles (N45, N32, N35 y N36) reportados en la literatura científica, donde se midió el porcentaje acumulado de liberación del fármaco a lo largo del tiempo.

Para estudiar el comportamiento del sistema, los datos fueron procesados en MATLAB mediante distintos modelos cinéticos:

- Modelo de Peppas, utilizado para describir mecanismos de difusión en matrices poliméricas.
- Modelo farmacocinético de primer orden, basado en liberación exponencial.
- Modelo empírico obtenido con Eureqa, ajustado directamente en función del tiempo.
- Modelo diferencial tipo Eureqa, resuelto numéricamente con Runge-Kutta de cuarto orden.eterminar y comparar la tasa de liberación controlada de fármacos en distintos hidrogeles poliméricos, con énfasis en el hidrogel N36-2MBA3, mediante el ajuste de datos experimentales usando modelos matemáticos no lineales, herramientas bioestadísticas y simulación in silico en MATLAB, con el fin de identificar el modelo que mejor describa la cinética de liberación acumulada del medicamento en función del tiempo.

## Actividades a realizar
1. Se importaron y organizaron en MATLAB los datos experimentales de tiempo y porcentaje de liberación de los cuatro hidrogeles.
2. Se graficaron las curvas experimentales para analizar el comportamiento de liberación acumulada del fármaco.
3. Se ajustaron los datos mediante modelos matemáticos no lineales: Peppas, farmacocinético de primer orden y modelos Eureqa.
4. Se aplicaron métodos numéricos y herramientas bioestadísticas para estimar parámetros y evaluar la calidad de cada ajuste.
5. Se compararon los resultados obtenidos para identificar el modelo más adecuado en la descripción de la liberación del hidrogel N36-2MBA3.
